...
public class Bullet : GameObject {
    ...
    public Bullet(Point2D center, Vector2D vector, int id) : base(center, vector, Radius) {
        PlayerId = id;
        Countdown = ConfigClass.Config["bullet"]["duration"].AsInt();
    }

    public Bullet(Point2D center, Vector2D vector, int id, double radius, int countdown) : base(center, vector, radius) {
        PlayerId = id;
        Countdown = countdown;
    }
    ...
    public static Bullet FromTextFormat(string input) {
        ...
        Bullet b = new(center, vec, Convert.ToInt32(items[0]), Convert.ToDouble(items[6]), Convert.ToInt32(items[5]));
